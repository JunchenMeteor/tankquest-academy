...
/data/
  projects/
    tankquest/
      cache/ai-wheelhouse/<content-key>/
...
`TankQuest Preview and Release` 在 PR 中验证 API/AI/Web 三个运行时镜像；合并到 `main` 或 `release` 后，Job 统一显示 `TankQuest / Deploy preview or release`，具体目标由 `Resolve target` 步骤记录为 `preview` 或 `release`。GitHub hosted runner 生成 Python 3.13 Linux wheelhouse 及逐文件 SHA-256 清单。TankQuest self-hosted runner 使用清单内容键复用 `/data/projects/tankquest/cache/ai-wheelhouse` 中严格校验通过的项目级依赖；只有缓存未命中时才下载提交 SHA 专属 wheelhouse artifact。源码和临时下载使用当前 Job 独有的 runner 临时目录，避免持久 workspace 残留进入 Docker build context。随后服务器在不访问 PyPI 的情况下构建带提交 SHA 的镜像并调用 `deploy/deploy-container.sh`。部署必须通过 Compose health check 和本机 `/api/health` 检查。
...
部署前脚本会快照上一版 `deployment.env` 和 Compose 文件。如果新容器失败，脚本原子恢复上一组 API/AI/Web 镜像与运行配置；首次增加 AI 容器时也能恢复到旧的双应用容器 Compose。数据库目录不会被部署或回滚流程删除。
