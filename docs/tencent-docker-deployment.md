...
`Deploy Tencent Docker` 在 PR 中验证 API/Web 镜像；合并到 `main` 或 `release` 后，由 TankQuest self-hosted runner 构建带提交 SHA 的镜像并调用 `deploy/deploy-container.sh`。部署必须通过 Compose health check 和本机 `/api/health` 检查。
