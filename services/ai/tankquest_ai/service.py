...
from .providers.template import (
    ...
    TemplateParentReportSummaryProvider,
...
@dataclass(frozen=True)
class ParentReportSummaryResult:
...
class ParentReportSummaryService:
    def __init__(
        self,
        *,
        fallback: ParentReportSummaryProvider,
        safety_guard: SafetyGuard,
        primary: ParentReportSummaryProvider | None = None,
        unavailable_reason: FallbackReason | None = None,
    ) -> None:
        self._fallback = fallback
        self._safety_guard = safety_guard
    ...
    def generate(self, request: ParentReportSummaryRequest) -> ParentReportSummaryResult:
        if self._primary is None:
            return ParentReportSummaryResult(
                payload=self._safe_fallback(request),
                source="template",
                fallback_reason=self._unavailable_reason,
            )
        ...
        try:
            payload = self._primary.generate(request)
            if not isinstance(payload, ParentReportSummaryPayload):
                logger.warning("ai_provider_fallback reason=invalid_output")
                return ParentReportSummaryResult(
                    payload=self._safe_fallback(request),
                    source="template",
            ...
            self._safety_guard.validate_parent_report_summary(payload)
            return ParentReportSummaryResult(payload=payload, source="model")
        except UnsafeDraftError:
            logger.warning("ai_provider_fallback reason=unsafe_output")
            return ParentReportSummaryResult(
                payload=self._safe_fallback(request),
    ...
    def _safe_fallback(self, request: ParentReportSummaryRequest) -> ParentReportSummaryPayload:
        payload = self._fallback.generate(request)
        self._safety_guard.validate_parent_report_summary(payload)
        return payload
...
def build_parent_report_summary_service(settings: Settings) -> ParentReportSummaryService:
    fallback = TemplateParentReportSummaryProvider()
    safety_guard = SafetyGuard()

    if settings.provider == "template":
        return ParentReportSummaryService(fallback=fallback, safety_guard=safety_guard)
    ...
    try:
        primary = LangChainOpenAIParentReportSummaryProvider(
            api_key=settings.openai_api_key,
            model=settings.model,
            timeout_seconds=settings.provider_timeout_seconds,
        )
...
def health_response(
    ...
    if settings.provider == "openai" and (
        ...
        or (parent_report_service is not None and not parent_report_service.uses_primary_provider)
    ):
